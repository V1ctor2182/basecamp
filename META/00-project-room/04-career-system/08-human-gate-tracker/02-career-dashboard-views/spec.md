...
## 当前进度 — m0/2 (planned 2026-05-10, 0%)
...
- ⏳ **m1-applied-page-full-rewrite** (~270 + smoke ~80) — Applied.tsx full rewrite. 30s polling of /applications. 7-chip filter strip (All / Applied / Responded / Interview / Offer / Rejected / Discarded). Per-row card with ScoreBadge + Role·Company + status pill + horizontal timeline stepper (each event = dot with hover tooltip) + followup highlight (≤3d yellow, past-due red) + Quick actions: [Open report], [Advance status] dropdown with VALID_TRANSITIONS legal-next-only + native confirm() + 400 with allowed_next surfaces inline, [Open job]. NEW applied.css with `ad-` prefix.
